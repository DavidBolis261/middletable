...
public class MovementR : MonoBehaviour
{
    ...
    {
        previousPosition = transform.position;
        if(!DOTween.IsTweening(transform) && !DOTween.IsTweening(otherPlayer.transform) && moveTimer >= duration && otherPlayer.GetComponent<MovementR>().moveTimer >= otherPlayer.GetComponent<MovementR>().duration)
        {
